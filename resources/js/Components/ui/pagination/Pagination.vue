  ...
  <nav aria-label="Pagination" class="flex items-center justify-between">
    ...
    <template v-for="(item, index) in paginationItems" :key="index">
      <button 
        ...
        class="w-10 h-10 p-0 text-sm font-medium text-gray-700 bg-gray-200 rounded-md hover:bg-gray-300"
      >
        {{ item.value }}
      </button>
      <span 
...
export default {
  ...
  computed: {
    ...
    paginationItems() {
      ...
      for (let i = Math.max(2, this.currentPage - this.siblingCount); i <= Math.min(this.totalPages - 1, this.currentPage + this.siblingCount); i++) {
